...
//
// Single-threaded in this phase: demux() decodes and routes, and
// process_pending() drains each shard's queue through its gap detector and
// the right symbol's book -- both called from the same thread here,
// simulating the feed per the build guide's explicit note. Real concurrent
// per-shard decoder threads are Phase 7's job.
...
// book pool's capacity (orderbook::kOrderPoolCapacity) -- this bounds
...
public:
    ...
    // Drains every shard's queue, feeding each message through that
    // shard's gap detector and the correct symbol's order book.
    ...
    // removed in process_pending(), after checking whether the book still
    ...
    // orders, not grow with total messages processed.
    std::size_t routing_table_size() const noexcept { return order_route_.size(); }
